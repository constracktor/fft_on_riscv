...
BASE_SIZE=16384
# Get run command
COMMAND="srun --cpu-bind=cores -N 1 -n 1 -c 1"
EXECUTABLE="../$1"
...
FFTW3_HPX_NTHREADS=1 OMP_PROC_BIND=CLOSE OMP_PLACES=cores$COMMAND $EXECUTABLE 1 $ARGUMENTS 1 
...
do
    COMMAND="srun --cpu-bind=cores -N 1 -n 1 -c $i"
    for (( j=0; j<$LOOP; j=j+1 ))
    do
        FFTW3_HPX_NTHREADS=$i OMP_PROC_BIND=CLOSE OMP_PLACES=cores $COMMAND $EXECUTABLE $i $ARGUMENTS 0
    done
done
# Log Info
date
